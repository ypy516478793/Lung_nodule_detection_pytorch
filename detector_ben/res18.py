from detector_ben.layers import *
...
class Net(nn.Module):
    def __init__(self, n_anchors=3):
        ...
        self.output = nn.Sequential(nn.Conv3d(self.featureNum_back[0], 64, kernel_size=1),
                                    nn.ReLU(),
                                    # nn.Dropout3d(p = 0.3),
                                    nn.Conv3d(64, 5 * self.n_anchors, kernel_size=1))
    ...
    def forward(self, x, coord):
        ...
        out = out.transpose(1, 2).contiguous().view(size[0], size[2], size[3], size[4], self.n_anchors, 5)
        # out = out.view(-1, 5)
        return out
