...
for s in wb.sheets():
    if s.name == 'list3.2':
        for row in range(1, s.nrows):
            ...
            assert abs(float(s.cell(row, 1).value) - int(s.cell(row, 1).value)) < 1e-8
            assert abs(float(s.cell(row, 2).value) - int(s.cell(row, 2).value)) < 1e-8
            assert abs(float(s.cell(row, 5).value) - int(s.cell(row, 5).value)) < 1e-8
            assert abs(float(s.cell(row, 6).value) - int(s.cell(row, 6).value)) < 1e-8
            assert abs(float(s.cell(row, 7).value) - int(s.cell(row, 7).value)) < 1e-8
...
valid_pids = []
for temp_pid in os.listdir(LIDCpath):
    if temp_pid.startswith('LIDC-IDRI-'):
        temp_pid = int(temp_pid.split('-')[2])
        valid_pids.append(temp_pid)
...
for k, v in antdict.items():
    ...
    if int(pid) < 10:
        sdu_temp_path = os.path.join(LIDCpath, 'LIDC-IDRI-000' + pid)
    elif int(pid) < 100:
        sdu_temp_path = os.path.join(LIDCpath, 'LIDC-IDRI-00' + pid)
    elif int(pid) < 1000:
        sdu_temp_path = os.path.join(LIDCpath, 'LIDC-IDRI-0' + pid)
    else:
        sdu_temp_path = os.path.join(LIDCpath, 'LIDC-IDRI-' + pid)

    for sdu in os.listdir(sdu_temp_path):
        for srs in os.listdir(os.path.join(*[sdu_temp_path, sdu])):
            if srs.endswith('.npy'):
            ...
            # if int(pid) == 380:
            #     print('check me out')
            if pid not in hmap_pid:
                hmap_pid[pid] = [srs]
            else:
                hmap_pid[pid].append(srs)
            ...
            if str(RefDs[0x20, 0x11].value) == scan or scan == '0':
                ...
                antdictscan[pid + '_' + RefDs[0x20, 0x0e].value] = v
                break
...
for srcid, lunaantlidc in lunantdictlidc.items():
    ...
    # either pid is not present or it is does not have 0000006.dcm
    if (int(pid) not in valid_pids) or (int(pid) in pid_lt6_dcms):
        continue
    ...
    for lunantdictlidcsub in lunaantlidc:
        ...
        if srcid in ['1.3.6.1.4.1.14519.5.2.1.6279.6001.174692377730646477496286081479',
            ...
            continue
        for idx, lidcant in enumerate(antdictscan[str(int(pid)) + '_' + srcid]):
