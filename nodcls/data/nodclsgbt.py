...
def readCSV(filename):
    ...
    with open(filename, "r") as f:
        ...
        for line in csvreader:
            lines.append(line)
    return lines
newlst = readCSV('annotationdetclsconvfnl_v3.csv')[1:]

preprocesspath = '../../LUNA16/preprocessed/lunaall/'
savepath = '../../LUNA16/cls/crop_v5/'  # v3: 17, v4: 36, v5: 32
import os
import os.path 
...
def crop_func(idx):
	...
	print(os.path.join(savepath, fname+'.npy'))

from multiprocessing import Pool
p = Pool(30)

_ = p.map(crop_func, list(range(len(newlst))))#3,4,1))#5,1))#1,9,1))
p.close()
...
# # train use gbt
# subset1path = '../../LUNA16/raw_files/subset1/'
# testfnamelst = []
