    ...
    def load_subset(self, subset):
        ...
        for folder in data_dir:
            for f in os.listdir(os.path.join(self.data_dir, folder)):
                if f.endswith('_image.npz') and f.strip("_image.npz") not in self.blacklist:
                    fileList.append(folder + '/' + f.strip("_image.npz"))

        self.filenames = [os.path.join(self.data_dir, "{:s}_image.npz".format(idx)) for idx in fileList]
        ...
        print(len(fileList))
        for idx in fileList:
            label_path = os.path.join(self.data_dir, "{:s}_label.npz".format(idx))
            l = np.load(label_path, allow_pickle=True)["label"].astype(np.float32)
            if np.all(l == 0):
                l = np.array([])
            labels.append(l) 
