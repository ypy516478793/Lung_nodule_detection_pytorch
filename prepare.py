...
Usage instructions:
    ...
    python prepare.py prep_methodist \
        ...
        -s=Methodist_incidental/data_Ben/masked_cropped_rawIntensity/ \
        --mask=True \
        --crop=True
...
config = {'train_data_path': ['LUNA16/raw_files/subset0/',
          ...
          'train_preprocess_result_path': 'LUNA16/masked_files/',
          ...
          'val_preprocess_result_path': 'LUNA16/masked_files/',
          ...
          'test_preprocess_result_path': 'LUNA16/masked_files/',
          ...
          'preprocess_result_path': 'LUNA16/raw_noNorm/',
...
def savenpy_luna_noNorm(id, annos, filelist, luna_segment, luna_data, savepath):
    islabel = True
    isClean = True
    resolution = np.array([1, 1, 1])
    #     resolution = np.array([2,2,2])
    ...
    if isClean:
        ...
        np.save(os.path.join(savepath, name+'_extendbox.npy'), extendbox)
        np.save(os.path.join(savepath, name+'_origin.npy'), origin)
        np.save(os.path.join(savepath, name+'_mask.npy'), dilatedMask)
...
def preprocess_luna():
    ...
    if not os.path.exists(finished_flag):
        ...
        for setidx in range(0, 10):
            ...
            # N = len(filelist)
            # for i in tqdm(range(N)):
            #     _ = savenpy_luna_noNorm(i, annos, filelist=filelist,
            #                          luna_segment=luna_segment, luna_data=luna_data + 'subset' + str(setidx) + '/',
            #                          savepath=savepath + 'subset' + str(setidx) + '/')

            partial_savenpy_luna_Masked = partial(savenpy_luna_noNorm, annos=annos, filelist=filelist,
                                                  luna_segment=luna_segment, luna_data=luna_data + 'subset' + str(setidx) + '/',
                                                  savepath=savepath + 'subset' + str(setidx) + '/')
            N = len(filelist)
            _ = pool.map(partial_savenpy_luna_Masked, range(N))
...
def make_lungmask(img, display=False, pad_value=-3000):
    raw_img = np.copy(img)
    ...
    for folder in os.listdir(root_dir):
        if os.path.isdir(os.path.join(root_dir, folder)):
            for f in os.listdir(os.path.join(root_dir, folder)):
                if f.endswith(".npz"):
                    data_ls.append(os.path.join(root_dir, folder, f))

    # ## single process
    # for data_path in tqdm(data_ls):
    #     mask_crop_func(data_path)

    # multi process
    from multiprocessing import Pool
    pool = Pool(32)
    pool.map(mask_crop_func, data_ls)
...
def rename():
    ''' Remove mrn for privacy concern'''
    path = os.walk("/home/cougarnet.uh.edu/pyuan2/Projects/DeepLung-3D_Lung_Nodule_Detection/Methodist_incidental/data_Ben/maskCropDebug")

    for root, directories, files in path:
        # for directory in directories:
        #     # print(directory)
        #     if len(directory) == 25 and directory.startswith("Lung_patient") and "-" in directory:
        #         old_directory = os.path.join(root, directory)
        #         new_directory = os.path.join(root, directory.split("-")[0])
        #         os.rename(old_directory, new_directory)
        #         print(new_directory)

        for file in files:
            if file.endswith("npz") and "-" in file:
                patient_id = root.split("/")[-1][5:15]
                old_name = os.path.join(root, file)
                mrn = file.split("-")[0]
                new_name = old_name.replace(mrn, patient_id)
                os.rename(old_name, new_name)
                print(new_name)

            # if file.endswith("_image.npz"):
            #     old_name = os.path.join(root, file)
            #     new_name = old_name.replace("_image.npz", "_clean.npz")
            #     os.rename(old_name, new_name)
            #     print(new_name)

def create_label():
    ''' Create label for methodist masked dataset'''
    annot_file = "./Methodist_incidental/data_Ben/resampled/pos_labels_norm.csv"
    root_dir = "/home/cougarnet.uh.edu/pyuan2/Projects/DeepLung-3D_Lung_Nodule_Detection/Methodist_incidental/data_Ben/masked"
    ...
    for root, directories, files in path:
        ...
        for file in files:
            if file.endswith("_clean.npz"):
                filename = file.split("_")[0]

                label_path = os.path.join(root, filename + "_label.npz")
                if os.path.exists(label_path):
                    continue

                extendbox = np.load(os.path.join(root, filename + "_extendbox.npz"))["extendbox"]
                ...
                np.savez_compressed(os.path.join(root, filename + "_label.npz"), label=label)

                print(os.path.join(root, filename + "_label.npz"))

def change_pad_values():
    root_dir = "/home/cougarnet.uh.edu/pyuan2/Projects/DeepLung-3D_Lung_Nodule_Detection/Methodist_incidental/data_Ben/masked"
    path = os.walk(root_dir)
    pad_value = -3000
    ...
    for root, directories, files in path:
        ...
        for file in files:
            ...
            if file.endswith("_clean.npz"):
                sliceim = np.load(os.path.join(root, file), allow_pickle=True)["image"]
                if len(sliceim) == 1 and len(sliceim.shape) == 4:
                    sliceim = sliceim[0]

                filename = file.split("_")[0]
                extendbox = np.load(os.path.join(root, filename + "_extendbox.npz"), allow_pickle=True)["extendbox"]
                mask = np.load(os.path.join(root, filename + '_mask.npz'), allow_pickle=True)["masks"]
                mask = mask[extendbox[0, 0]:extendbox[0, 1],
                       extendbox[1, 0]:extendbox[1, 1],
                       extendbox[2, 0]:extendbox[2, 1]]
                sliceim = sliceim * mask + pad_value * (1 - mask).astype('uint8')

                np.savez_compressed(os.path.join(root, file), image=sliceim[np.newaxis, ...])

                print(os.path.join(root, file))

# def creat_annotations():
#     ''' Create annotations.csv for preprocessed luna data'''
#     data_dir = "/home/cougarnet.uh.edu/pyuan2/Projects/DeepLung-3D_Lung_Nodule_Detection/LUNA16/masked_files"
#     annotations = [['seriesuid', 'coordX', 'coordY', 'coordZ', 'diameter_mm']]
#
#     subsets = os.listdir(data_dir)
#     for subset in subsets:
#         subdir = os.path.join(data_dir, subset)
#         label_files = [i for i in os.listdir(subdir) if "label" in i]
#         for file in label_files:
#             filename = file.split("_")[0]
#             if file.endswith(".npy"):
#                 label = np.load(os.path.join(subdir, file))
#             else:
#                 assert file.endswith(".npz")
#                 label = np.load(os.path.join(subdir, file), allow_pickle=True)["image"]
#             if not np.all(label == 0):
#                 for l in label:
#                     l[:3] = l[:3][::-1]
#                     l = [filename, ] + l.tolist()
#                     annotations.append(l)
#     save_path = os.path.join(data_dir, "annotations.csv")
#     np.savetxt(save_path, annotations, delimiter=",")
...
if __name__ == '__main__':
    ...
    parser.add_argument('command', choices=["prep_luna", "prep_methodist", "ch_infopath", "ass_pet", "get_info", "rename", "create_label", "create_pad",
                                            "raw2norm", "check_label", "extract", "convert", "update_details", "update_checklist"],
                        help="options: [prep_luna, prep_methodist, ch_infopath, ass_pet, get_info, raw2norm, create_pad"
                             "check_label, extract, convert, update, rename, create_label]", default="prep_methodist")
